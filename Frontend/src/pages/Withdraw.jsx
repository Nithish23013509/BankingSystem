...
export default function Withdraw() {
  ...
  const insufficientFunds = selectedAccount && numericAmount > 0 && numericAmount > selectedAccount.balance;
  ...
  const validate = useCallback(() => {
    ...
    if (selectedAccount && numericAmount > selectedAccount.balance) {
      errs.amount = 'Insufficient funds in this account';
    }
    return errs;
  }, [selectedAccountId, amount, numericAmount, selectedAccount]);
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const result = await accountService.withdraw(selectedAccountId, numericAmount, description);
      addNotification(`Successfully withdrew ${formatCurrency(numericAmount)}`, 'success');
      setSuccess({
        ...result,
      ...
      if (err.message === 'Insufficient funds') {
        addNotification('Insufficient funds. The withdrawal amount exceeds your balance.', 'error');
        setErrors({ amount: 'Insufficient funds' });
      } else {
        addNotification(err.message || 'Withdrawal failed. Please try again.', 'error');
      }
    } finally {
      setSubmitting(false);
    }
          ...
          <div className="success-state">
            ...
            <div className="transaction-summary">
              ...
              <div className="summary-row total">
                <span className="label">Amount Withdrawn</span>
                <span className="value" style={{ color: '#ff6b6b', fontSize: 18, fontWeight: 700 }}>
    ...
    <div className="transaction-page">
      <div className="page-header">
        ...
        <p className="page-subtitle">
          Withdraw funds from any of your active accounts.
        </p>
      </div>
        ...
        <form onSubmit={handleSubmit} noValidate>
              ...
              {numericAmount > 0 && (
                <>
                  <div className="balance-row">
                    ...
                    <span className="value negative">−{formatCurrency(numericAmount)}</span>
                  </div>
                  <div className="balance-row">
                    ...
                    <span
                      className={`value ${insufficientFunds ? 'negative' : ''}`}
                      style={{ fontSize: 17 }}
                    >
                      {insufficientFunds ? '−' : ''}{formatCurrency(Math.abs(balanceAfter))}
                    </span>
                  </div>
                </>
              )}
          ...
          {insufficientFunds && (
            <div className="insufficient-warning">
              ...
              Insufficient funds — your withdrawal amount exceeds the available balance
              of {formatCurrency(selectedAccount.balance)}.
            </div>
          )}
          ...
          <div className="form-actions">
            ...
            <button
              type="submit"
              className="btn-submit btn-withdraw"
